...
public class MediaClient(HttpClient httpClient)
{
    ...
    public string BuildDownloadUrl(string url, string type, string quality = "best")
    {
        if (httpClient.BaseAddress is null)
            throw new InvalidOperationException("HttpClient BaseAddress is not configured.");

        var encodedUrl = Uri.EscapeDataString(url);
        var encodedQuality = Uri.EscapeDataString(quality ?? "best");
        return new Uri(httpClient.BaseAddress, $"medias/download?url={encodedUrl}&type={type}&quality={encodedQuality}").ToString();
    }
}
